...
public class ProbabilityTests
{
    ...
    public void Basic()
    {
        ...
        hand = new List<Card> { new (Suit.Hearts, Rank.Ace), new (Suit.Clubs, Rank.Jack), new (Suit.Spades, Rank.Jack), 
    ...
    public void AllValues()
    {
        ...
        for (var i = 0; i <= 11; i++)
        {
            Assert.That(nextValueProbs[i], Is.EqualTo(0));
        }
        
        for (var i = 12; i < nextValueProbs.Length; i++)
        {
            Assert.That(nextValueProbs[i], Is.InRange(0, 1));
        }
    }
}